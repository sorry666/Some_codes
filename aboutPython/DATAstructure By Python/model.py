...
class PFEvaluator(object):
	...
	def evaluate(self):
		while self._scanner.hasNext:
			...
			if currentToken.getType() == Token.INT:
				...
				if len(self._operandStack) < 2:
					raise AttributeError ("too few oprands on the stack ")
		...
		if len(self._operandStack) > 1:
			raise AttributeError ("Too many oprand on the stack")
	...
	def __str__(self):
		...
		if self._expressionSoFar == "":
			result += "Portion of expression prosceed :none \n"
		else:
			result += "Portion of expression prosceed : " +self._expressionSoFar +"\n"
		if self._operandStack.isEmpty():
			result = "The stack is empty"
		else:
			result += "Oprands on the stack :" +str(self._operandStack)
		return result
